...
public class ReqGetByInnTp {
    public static LinkedList<TradingPointTemplate> loadWithAnotherProject(String INN) {
        for (int i = 0; i < C.SOAP.URLs.length; i++) {
            if (!C.SOAP.URLs[i].equals(AppCache.USER_INFO.getProjectURL())) {
                LinkedList<TradingPointTemplate> list = load(INN);
                if(!list.isEmpty()){
                    return list;
                }
            }
        }
        LinkedList<TradingPointTemplate> list =  new LinkedList<>();
        return list;
    }

    public static LinkedList<TradingPointTemplate> load(String INN) {
        final String soapAction = "http://www.sample-package.org#MobileAgents:GetClientsThroughINN";
        final String methodName = "GetClientsThroughINN";

        SoapObject request = new SoapObject(C.SOAP.NAME_SPACE, methodName);
        request.addProperty("INN", INN);
        ...
        try {
            ...
            for (int i = 0; response.getPropertyCount() > i; i++) {
                ...
                temp = item.getProperty("MFO").toString();
                tp.setMfo((temp.equalsIgnoreCase("anyType{}") ? "" : temp));

                temp = item.getProperty("BankAccount").toString();
                tp.setRc((temp.equalsIgnoreCase("anyType{}") ? "" : temp));
                ...
                temp = item.getProperty("Director").toString();
                tp.setDirector((temp.equalsIgnoreCase("anyType{}") ? "" : temp));

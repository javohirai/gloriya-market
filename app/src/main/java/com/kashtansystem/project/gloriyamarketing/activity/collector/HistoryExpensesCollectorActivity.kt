...
class HistoryExpensesCollectorActivity: BaseKActivity(){
    ...
    override fun init(bundle: Bundle?) {
        ...
        etFromDate.setOnClickListener{
            ...
            if (DATE_FROM != -1L)
                date.time = DATE_FROM
            calendar.time = date
            val datePickerDialog = DatePickerDialog(this@HistoryExpensesCollectorActivity, DatePickerDialog.OnDateSetListener { view, year, month, dayOfMonth ->
    ...
    fun updateFragments(){
        supportFragmentManager.fragments.forEach {
            if(it is HistoryExpensesCollectorListFragment){
    ...
    inner class SimpleFragmentPagerAdapter(private val mContext: Context, fm: FragmentManager) : FragmentPagerAdapter(fm) {
        ...
        override fun getItem(position: Int): Fragment {
            var fragment = HistoryExpensesCollectorListFragment()
            val bundle = Bundle()
